...
class AbstractRemindablesService
{
    public static function get(RemindablesQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new RemindablesQueryFilter($request);
        }
    ...
    {
        $object = Remindables::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\Communication\\Actions\\Remindables\\' . Str::studly($action);
